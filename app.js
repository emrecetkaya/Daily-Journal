...
app.get("/", function (req, res) {
  res.render("home", { startingContent: homeStartingContent, postArray: postList })
...
app.get("/posts/:pageId", function (req, res) {
  ...
  const requestedTitle = loDash.lowerCase(req.params.pageId);

  postList.forEach(function (post) {
    const storedTitle = loDash.lowerCase(post.postTitle);

    if (storedTitle === requestedTitle) {
      res.render("post", { postId: post.postTitle, post: post.postBody });
    }
  })
